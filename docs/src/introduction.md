...
Claude Code (from [numtide/llm-agents.nix](https://github.com/numtide/llm-agents.nix)) runs inside an isolated sandbox with filesystem isolation, display forwarding, and a Chromium browser. Three backends are available with increasing isolation strength:
